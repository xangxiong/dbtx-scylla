...
from dbt.adapters.scylla import ScyllaConnectionManager
from dbt.adapters.scylla.column import ScyllaColumn
from dbt.adapters.scylla import ScyllaRelation
...
GET_RELATIONS_MACRO_NAME = "scylla__get_relations"


@dataclass
class ScyllaIndexConfig(dbtClassMixin):
    columns: List[str]
    unique: bool = False
    ...
    @classmethod
    def parse(cls, raw_index) -> Optional["ScyllaIndexConfig"]:
...
@dataclass
class ScyllaConfig(AdapterConfig):
    unlogged: Optional[bool] = None
    indexes: Optional[List[ScyllaIndexConfig]] = None


class ScyllaAdapter(SQLAdapter):
    Relation = ScyllaRelation
    ConnectionManager = ScyllaConnectionManager
    Column = ScyllaColumn

    AdapterSpecificConfigs = ScyllaConfig
    ...
    @available
    def parse_index(self, raw_index: Any) -> Optional[ScyllaIndexConfig]:
        return ScyllaIndexConfig.parse(raw_index)
    ...
    def _get_catalog_schemas(self, manifest):
        # Scylla only allow one database (the main one)
