...
from dbt.adapters.scylla.relation_configs.constants import MAX_CHARACTERS_IN_IDENTIFIER
from dbt.adapters.scylla.relation_configs.index import (
    ScyllaIndexConfig,
...
class ScyllaMaterializedViewConfig(RelationConfigBase, RelationConfigValidationMixin):
    """
    ...
    table_name: str = ""
    query: str = ""
    indexes: FrozenSet[ScyllaIndexConfig] = field(default_factory=frozenset)
    ...
    @classmethod
    def from_dict(cls, config_dict: dict) -> "ScyllaMaterializedViewConfig":
        kwargs_dict = {
            "table_name": config_dict.get("table_name"),
            "query": config_dict.get("query"),
            "indexes": frozenset(
                ScyllaIndexConfig.from_dict(index) for index in config_dict.get("indexes", {})
            ),
        }
        materialized_view: "ScyllaMaterializedViewConfig" = super().from_dict(kwargs_dict)  # type: ignore
        return materialized_view

    @classmethod
    def from_model_node(cls, model_node: ModelNode) -> "ScyllaMaterializedViewConfig":
    ...
    def parse_model_node(cls, model_node: ModelNode) -> dict:
        ...
        config_dict = {
            ...
            "indexes": [ScyllaIndexConfig.parse_model_node(index) for index in indexes],
        }
        return config_dict
    ...
    def parse_relation_results(cls, relation_results: RelationResults) -> dict:
        ...
        config_dict = {
            "indexes": [
                ScyllaIndexConfig.parse_relation_results(index) for index in indexes.rows
            ],
        }
        return config_dict
...
@dataclass
class ScyllaMaterializedViewConfigChangeCollection:
    indexes: Set[ScyllaIndexConfigChange] = field(default_factory=set)
